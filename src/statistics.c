...
void run_statistics(void)
{
    ...
    while (1)
    {
        ...
        // Extract miner ID
        int miner_id = 0;
        if (sscanf(msg.block.txb_id, "MINER-%d-", &miner_id) != 1 ||
            miner_id < 1 || miner_id > max_miners)
        {
            miner_id = 0;
        }
        ...
        for (int id = 1; id <= max_miners; id++) {
            printf("Miner %d: valid=%d  invalid=%d  credits=%d\n",
                   id, valid_blocks[id], invalid_blocks[id], valid_blocks[id]);
        }
        double avg_tx = total_tx_count ?
            (double)total_tx_verify_time / total_tx_count : 0.0;
